...
for ticker in tickers:
    ...
    if re.search('2018-1\d-\d\d', bs["financials"][3]['date']):
        ...
        print(f"No hubo match para {ticker}")
        a,b,c = 6,10,14
...
def return_with_divs(tickers):
    ...
    for ticker in tickers:
        ...
        print(f'{ticker} variation was {round(variation*100,2)}%')
    return rend2

F_score["return+div"] = return_with_divs(tickers)
...
plt.title(tickers)
plt.xlabel("F-Score 2018-dic", color="black")
plt.ylabel("Retorno posterior", color="black")
plt.show()
